...
class CellGenerator():
    def __init__(self, path_input_yaml):
        if type(path_input_yaml) == dict:
            inputs = path_input_yaml
        elif type(path_input_yaml) == str:
            with open(path_input_yaml, 'r') as f:
                inputs = yaml.safe_load(f)
        else:
            raise TypeError("Input should be a dictionary or a string")
        ...
        if inputs['path'].get('path_dst'):
            self.dst = Path(inputs['path']['path_dst'])
        else:
            self.dst = Path('.')
    ...
    def _write(self):
        ...
        write(self.dst/'POSCAR', result, format='vasp', sort=True)
