...
class MeanSquaredDisplacementPlotter:
    ...
    def run(self):
        ...
        if not os.path.exists(self.dst):
            ...
            print(f"File {self.dst} already exists. Skip calculating MSD.")
    ...
    def _load_data(self):
        ...
        atoms_list = read(self.src, index=f'::{self.step}', format=atoms_format)
        idx_to_remove = self._filter_by_height(atoms_list, self.height_cutoff)

        initial_image = atoms_list[0]
        idx_F = np.array([
            atom.index for atom in initial_image
            if atom.symbol == self.atom_type and atom.index not in idx_to_remove])
    ...
    @staticmethod
    def _filter_by_height(atoms_list, height_cutoff):
        idx_to_remove = set()
        cutoff = atoms_list[0].get_cell()[2, 2] + height_cutoff
